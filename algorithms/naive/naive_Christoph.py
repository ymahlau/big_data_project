...
def main():
    ...
    result = pd.DataFrame()
    result["result_table_name"] = list(
        map(lambda x: x[1].columns.name, reorderd_tables)
    )
    ...
    result["joinability"] = list(map(lambda x: x[6], reorderd_tables))
...
def load_query():
    ...
    return pd.read_csv("../../data/toy_data/master/A.csv", sep=";")
...
def load_tables():
    ...
    tables = []
    for path in Path("../../data/toy_data").glob("*.csv"):
        ...
        for query_column in query_cols:
            for result_column in table.select_dtypes(include=["object"]).columns:
                ...
                if joinability > 0:  # Todo: maybe add threshold
                    joinability_with_column.append(
                        (joinability, table, query_column, result_column)
                    )
...
def rank_tables(query, tables):
    ...
    for joinability, table, col, col2 in tables:
        joined = query.add_suffix("_q").merge(
            table.add_suffix("_t"),
            left_on=col + "_q",
            right_on=col2 + "_t",
            how="inner",
        ...
        for query_col, table_col in product(left_cols, right_cols):
            ...
            reorderd.append((corr, table, col, col2, query_col, table_col, len(joined)))
